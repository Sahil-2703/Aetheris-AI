    ...
    try {
      await signIn.email(
        ...
        },
        {
          ...
          onError: (ctx) => {
            setError(ctx.error.message || "Invalid email or password. Please verify your credentials.");
          },
        }
      );
    } catch (err: any) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (checkRes.ok) {
        ...
        if (!providers.google) {
          setError(
            "Google OAuth is not configured yet. Please add your GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET into the .env file, or use Email & Password sign-in."
          );
          return;
        }
      }
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#050713] px-6 py-12 text-slate-100 selection:bg-purple-600 selection:text-white">
      <div className="w-full max-w-md space-y-8 rounded-3xl border border-purple-500/30 bg-[#090c24]/90 p-8 shadow-2xl shadow-purple-950/40 backdrop-blur-xl">
        <div className="text-center">
          ...
          <p className="mt-1 text-xs text-slate-400">Authenticate to enter your role workspace</p>
        </div>
        ...
        <p className="text-center text-xs text-slate-400">
          ...
          <Link href="/sign-up" className="font-semibold text-purple-400 hover:underline">
            Create account
          </Link>
        </p>
      </div>
